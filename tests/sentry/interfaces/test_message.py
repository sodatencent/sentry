...
class MessageTest(TestCase):
    ...
    def test_serialize_non_string_for_message(self):
        result = type(self.interface).to_python(True, {
            'message': {
                'foo': 'bar'
            },
        })
    ...
    def test_retains_formatted(self):
        result = type(
            self.interface).to_python(
            True, {
                'message': 'foo bar', 'formatted': 'foo bar baz'})
        assert result.message == 'foo bar'
    ...
    def test_discards_dupe_formatted(self):
        result = type(
            self.interface).to_python(
            True, {
                'message': 'foo bar', 'formatted': 'foo bar'})
        assert result.message == 'foo bar'
