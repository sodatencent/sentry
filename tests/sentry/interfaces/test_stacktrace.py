...
class StacktraceTest(TestCase):
    @fixture
    def interface(self):
        return Stacktrace.to_python(True,
                                    dict(
                                        frames=[
                                            {
                                                'filename': 'foo/bar.py'
    ...
    def test_legacy_interface(self):
        ...
        # objects
        event = self.event
        interface = Stacktrace.to_python(True, event.data['sentry.interfaces.Stacktrace'])
    ...
    def test_requires_filename(self):
        with self.assertRaises(InterfaceValidationError):
            Stacktrace.to_python(True, dict(frames=[{}]))
    ...
    def test_requires_frames(self):
        with self.assertRaises(InterfaceValidationError):
            Stacktrace.to_python(True, {})

        with self.assertRaises(InterfaceValidationError):
            Stacktrace.to_python(True, dict(frames=[]))

        with self.assertRaises(InterfaceValidationError):
            Stacktrace.to_python(True, dict(frames=1))

    def test_allows_abs_path_without_filename(self):
        interface = Stacktrace.to_python(True,
                                         dict(frames=[{
                                             'lineno': 1,
    ...
    def test_serialize_returns_frames(self):
        interface = Stacktrace.to_python(True, dict(frames=[{
            'lineno': 1,
            'filename': 'foo.py',
        }]))
    ...
    def test_get_hash_with_minimal_app_frames(self):
        ...
        interface = Stacktrace.to_python(True, dict(frames=frames))
        result = interface.get_hash(system_frames=False)
        assert not result

    def test_get_hash_with_only_required_vars(self):
        interface = Frame.to_python(True, {
            'lineno': 1,
            'filename': 'foo.py',
        })
    ...
    def test_get_hash_uses_module_over_filename(self):
        interface = Frame.to_python(True, {'lineno': 1, 'filename': 'foo.py', 'module': 'foo'})
        result = interface.get_hash()
        self.assertEquals(result, ['foo', 1])

    def test_get_hash_uses_function_over_lineno(self):
        interface = Frame.to_python(True, {'lineno': 1, 'filename': 'foo.py', 'function': 'bar'})
        result = interface.get_hash()
    ...
    def test_cocoa_culprit(self):
        stacktrace = Stacktrace.to_python(True,
                                          dict(
                                              frames=[
                                                  {
                                                      'filename': 'foo/baz.c',
    ...
    def test_to_string_returns_stacktrace(self, get_stacktrace):
        ...
        interface = Stacktrace(True, frames=[])
    ...
    def test_get_traceback_response(self, get_stacktrace):
        ...
        interface = Stacktrace.to_python(True, dict(frames=[{'lineno': 1, 'filename': 'foo.py'}]))
    ...
    def test_get_stacktrace_with_only_filename(self):
        event = mock.Mock(spec=Event())
        interface = Stacktrace.to_python(True, dict(
            frames=[{'filename': 'foo'}, {'filename': 'bar'}]))
    ...
    def test_get_stacktrace_with_module(self):
        event = mock.Mock(spec=Event())
        interface = Stacktrace.to_python(True, dict(frames=[{'module': 'foo'}, {'module': 'bar'}]))
        result = interface.get_stacktrace(event)
        self.assertEquals(
            result, 'Stacktrace (most recent call last):\n\n  Module "foo"\n  Module "bar"'
        )
...
class SlimFrameDataTest(TestCase):
    def test_under_max(self):
        interface = Stacktrace.to_python(True, {'frames': [{'filename': 'foo'}]})
    ...
    def test_over_max(self):
        ...
        interface = Stacktrace.to_python(True, {'frames': values})
        slim_frame_data(interface, 4)
